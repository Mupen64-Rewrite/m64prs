...
impl ParseCallbacks for M64PParseCallbacks {
    fn enum_variant_name(
        ...
        match _enum_name {
            Some("m64p_GLContextType") => Some(_original_variant_name[24..].to_owned()),
            Some("m64p_GLattr") => Some(_original_variant_name[8..].to_owned()),
            Some("m64p_render_mode") => Some(_original_variant_name[12..].to_owned()),
            Some(_) => {
                let underscore_pos = _original_variant_name.find('_').unwrap();
                Some(_original_variant_name[(underscore_pos + 1)..].to_owned())
            }
            _ => None,
        }
    }
